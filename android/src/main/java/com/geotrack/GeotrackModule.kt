...
class GeotrackModule(private val reactContext: ReactApplicationContext) :
    NativeGeotrackSpec(reactContext) {

    private var platformBridge: GeotrackPlatformBridge? = null

    private fun getBridge(): GeotrackPlatformBridge {
        if (platformBridge == null) {
            platformBridge = GeotrackPlatformBridge(reactContext).also { bridge ->
                bridge.locationService = GeotrackLocationService(reactContext, bridge)
            }
        }
    ...
    override fun configure(config: String, promise: Promise) {
        try {
            ...
            promise.reject("GEOTRACK_ERROR", e.message, e)
        }
    }
    ...
    override fun start(promise: Promise) {
        try {
            val bridge = getBridge()
            GeotrackForegroundService.start(reactContext)
            bridge.start()
            promise.resolve(bridge.getState())
        } catch (e: Exception) {
            promise.reject("GEOTRACK_ERROR", e.message, e)
    ...
    override fun stop(promise: Promise) {
        try {
            val bridge = getBridge()
            bridge.stop()
            GeotrackForegroundService.stop(reactContext)
            promise.resolve(bridge.getState())
        } catch (e: Exception) {
            promise.reject("GEOTRACK_ERROR", e.message, e)
        }
    ...
    companion object {
        const val NAME = NativeGeotrackSpec.NAME
    }
}
