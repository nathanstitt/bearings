...
Geomony is a React Native Turbo Module for battery-conscious background location tracking and geofencing. The core logic is implemented in shared C++17, with thin platform bridges for iOS (Objective-C++) and Android (Kotlin + JNI).
...
1. **C++ core** (`cpp/`) — All business logic: state machine, location storage (SQLite), config management. Platform-agnostic.
   - `GeomonyCore` — Main entry point. Owns the state machine (UNKNOWN → MOVING ↔ STATIONARY), config, and location store.
...
2. **Platform bridges** — Implement `PlatformBridge` in native code:
   - **iOS** (`ios/`) — `GeomonyPlatformBridge.mm` implements the C++ `PlatformBridge`. `GeomonyLocationDelegate` wraps `CLLocationManager`. `Geomony.mm` is the Turbo Module entry point.
   - **Android** (`android/`) — `GeomonyPlatformBridge.kt` calls C++ via JNI (`android/src/main/cpp/geomony_jni.cpp`). `GeomonyLocationService.kt` uses FusedLocationProvider. `GeomonyForegroundService.kt` keeps tracking alive in background. `GeomonyModule.kt` is the Turbo Module entry point.
...
3. **TypeScript layer** (`src/`) — Thin wrapper over the Turbo Module. All native methods exchange JSON strings which are parsed/serialized at the JS boundary.
   - `NativeGeomony.ts` — TurboModule spec (codegen input).
...
- **Vendored C dependencies** in `cpp/vendor/`: SQLite (`sqlite3/`) and nlohmann/json (`nlohmann/json.hpp`). Both are compiled/included with specific flags (see `CMakeLists.txt` and `Geomony.podspec`). Use nlohmann/json for all JSON construction and parsing in C++ code.
